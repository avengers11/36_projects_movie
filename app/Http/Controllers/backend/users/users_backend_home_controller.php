...
class users_backend_home_controller extends Controller
{
    ...
    // users_home_change_password_controller
    public function users_home_change_password_controller(Request $req)
    {
        $data = $req -> all();
        $userData = users::where('username', $req -> session() -> get('username')) -> first();
        if($req->old_pass != $userData['password']){
            return back()->with('msg', "Password not match! ");
        }
        users::where('username', $req -> session() -> get('username')) -> update([
            'password' => $req->new_pass
        ]);
        return back()->with('msg', "CONGRATULATIONS! ");
    }
